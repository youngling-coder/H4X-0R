...
async def command_start_handler(message: Message) -> None:
    ...
    if username == h4x0r_settings.OWNER_USERNAME:
        ...
        if not chat:
            chat = create_new_chat(title=username, type_="private", participants={
                username: full_name
            })
        ...
        await message.reply(
            escape_markdown(response), reply_markup=menu_keyboard_markup, parse_mode=ParseMode.MARKDOWN
        )
...
async def message_handler(message: Message) -> None:
    ...
    if chat_type in ("group", "supergroup") and message.text:
        ...
        if not chat:
            ...
            participants = {
                participant.username if participant.username else str(participant.id): f"{participant.first_name} {participant.last_name}" for participant in participants
            }
            chat = create_new_chat(title=title, type_=chat_type, participants=participants)

        message_text = message.text
        message_text = f"message by: {message.from_user.username} ({message.from_user.first_name} {message.from_user.last_name})\n\n" + message_text
