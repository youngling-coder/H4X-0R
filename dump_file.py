...
def read_dump_file(filename: str):
    ...
    dump_file_path = os.path.join(h4x0r_settings.DUMP_FILES_FOLDER, filename)
...
def write_dump_file(filename: str, obj: Any) -> None:
    ...
    dump_files_folder_exists = os.path.exists(h4x0r_settings.DUMP_FILES_FOLDER)

    if not dump_files_folder_exists:
        try:
            os.mkdir(h4x0r_settings.DUMP_FILES_FOLDER)
